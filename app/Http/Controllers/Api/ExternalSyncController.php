...
class ExternalSyncController extends Controller
{
    ...
    public function syncCommande(Request $request): JsonResponse
    {
        ...
        $integrationClient = $request->attributes->get('integration_client');
        $this->audit(
            'integration.sync_commande',
            'demandes_reservations',
            (int) $demande->id,
            null,
            [
                'numero_commande' => $data['numero_commande'],
                'was_recently_created' => $demande->wasRecentlyCreated,
                'integration_client_id' => $integrationClient?->id,
                'integration_client_key' => $integrationClient?->client_id,
            ],
        );
    ...
    public function storeLivraison(Request $request): JsonResponse
    {
        ...
        $integrationClient = $request->attributes->get('integration_client');
        $this->audit(
            'integration.store_livraison',
            'livraisons',
            (int) $result['livraison']->id,
            null,
            [
                'vin' => $data['vin'],
